...
if(isset($_POST['update'])){
   ...
   $is_free = $_POST['is_free']; // New field to indicate if content is free or paid
   $is_free = filter_var($is_free, FILTER_SANITIZE_STRING);

   $update_content = $conn->prepare("UPDATE `content` SET title = ?, description = ?, status = ?, is_free = ? WHERE id = ?");
   $update_content->execute([$title, $description, $status, $is_free, $video_id]);
...
<section class="video-form">
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <p>Is this content free? <span>*</span></p>
      <select name="is_free" class="box" required>
         <option value="<?= $fecth_videos['is_free']; ?>" selected><?= $fecth_videos['is_free'] ? 'Yes' : 'No'; ?></option>
      ...
      <div class="flex-btn">
         ...
         <input type="submit" value="delete" name="delete_video" class="delete-btn" onclick="return confirm('Delete this content?');">
      </div>
   </form>
   <?php
         }
